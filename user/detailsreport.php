...
$routtel=$mysqli->query("SELECT *
                            FROM employee
                            INNER JOIN emp_tel
                            ON employee.emp_id=emp_tel.emp_id
                            WHERE employee.emp_id='$emp_id' AND  emp_tel.tel = '$tel'");
while($row = $routtel->fetch_assoc())
{
    $rout = $row['route'];
}
//search tel map
$sql=$mysqli->query("SELECT route_tel.eq_id,route_tel.label,route_tel.type,route_tel.slot,equipment.location
                    FROM route_tel
      ...
      <div class="container-fluid">
            <div class="row justify-content-center">
              ...
              ?>
                            ...
                            $sql=$mysqli->query("SELECT * 
                            FROM emp_tel
                            WHERE tel='$tel' AND emp_id ='$emp_id'");
                            while($row = $sql->fetch_assoc())
                            {
                                ...
                                <div class="col-12">
                                                ...
                                                <?php
                                                    ...
                                                    <div class ="row">
                                                        <div class= "col-4">
                                                         ...
                                                         <label><?php echo $row['re_type']?></label>  
                                                        </div>
                                                    </div>
                                                    <div class ="row">
                                                        <div class= "col-4">
                                                         <label>วันที่ซ่อม :</label>  
                                                         <label><?php echo $row['re_date']?></label>  
                                                        </div>
                                                    </div>
                                                    <div class ="row">
                                                        <div class= "col-4">
                                                         <label>รายละเอียด :</label>  
                                                         <label><?php echo $row['re_data']?></label>  
                                                        </div>
                                                    </div>
                                                    <?php
                                                    $Worker=$row['emp_id'];
                                    ...
                                    <div class="form-group">
                                        <div class="row justify-content-center">
                                            ...
                                            <a href="showreport2.php?Start_date=<?php echo$_SESSION['Start_date']?>&End_date=<?php echo $_SESSION['End_date']?>&type=<?=$_SESSION['type'];?>" class="btn btn-facebook">ย้อนกลับ</a>
                                        </div>
                                    </div>
                                </div>
                            </div>
